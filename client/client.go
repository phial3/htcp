...
func NewClient(opts ...Option) (*Client, error) {
	...
	if options.ConnectAddr == "" {
		return nil, utils.AssertError{"未设置 ConnectAddr ,请使用 WithConnectAddr(addr)"}
	}
...
func (m *Client) Send(data []byte) (err error) {
	if m.Status() != config.ClientConnected {
		return utils.AssertError{"Client 非 ClientConnected 状态时不能使用 Send"}
	}
	if len(data) == 0 {
		return
	}
...
func (m *Client) connectedHandler() {
	...
	var trustTime = time.NewTicker(config.DefaultWaitTrustTime)
...
func (m *Client) heartbeatFunc(timer *utils.HeartbeatTime) {
	if m.opts.IsServerClient {
		...
		// 发送一个空数据来表示心跳
		n, err := m.opts.Conn.Write(make([]byte, config.DataHeaderLength))
		if err != nil {
			utils.Logger.Errorf("heartbeat error: %v", err)
			return
		}
		utils.Logger.Debugf("heartbeat send %d bytes", n)
	}
}
